...
void sdsfree(sds s) {
    if (s == NULL) return;
    // 释放内存的时候，需要计算出sds头部的位置，否则可能导致内存泄露
 ...
 * 扩容sds
 * addlen 需要扩大的长度
 * greedy=1 会分配比需要的更大的空间，新长度小于SDS_MAX_PREALLOC的，分配两倍，大于SDS_MAX_PREALLOC的，分配SDS_MAX_PREALLOC的冗余空间
 */
 ...
 */
/*
 * 模版sds实例化，比如 {title}xxx，遇到{title}会回调cb_func
 */
